...
class Quaternion
{
	...
	inline void Multiply(const Quaternion q)
	{
		Quaternion tmp;
		tmp.u.x =
		    ((w * q.u.x) + (u.x * q.w) + (u.y * q.u.z) - (u.z * q.u.y));
		tmp.u.y =
		    ((w * q.u.y) - (u.x * q.u.z) + (u.y * q.w) + (u.z * q.u.x));
		tmp.u.z =
		    ((w * q.u.z) + (u.x * q.u.y) - (u.y * q.u.x) + (u.z * q.w));
		tmp.w =
		    ((w * q.w) - (u.x * q.u.x) - (u.y * q.u.y) - (u.z * q.u.z));
		*this = tmp;
	}
	...
	{
		double norm = sqrt(u.x * u.x + u.y * u.y + u.z * u.z + w * w);
		u.x /= norm;
		u.y /= norm;
		u.z /= norm;
	}
...
void display(void)
{
	...
	glRasterPos2f(50, 30);
	sprintf(buffer,
		"Spherical Linear Interpolation of Quaternions (L=%.3f)",
		lambdaanim);
...
inline void Spherical2Cartesian(double t, double p, double &X, double &Y,
				double &Z)
{
	X = cos(p) * sin(t);
	Y = sin(p);
...
void keyboard(unsigned char key, int x, int y)
{
	...
	Spherical2Cartesian(M_PI * drand() * 2.0, -M_PI / 2.0 + M_PI * drand(),
			    p.x, p.y, p.z);
...
int _tmain(int argc, _TCHAR *argv[])
{
	...
	cout << "Visual Computing: Geometry, Graphics, and Vision "
		"(ISBN:1-58450-427-7)"
	     << endl;
	cout << "Demo program\n\n" << endl;
